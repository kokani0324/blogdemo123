...
### 流程（現行架構：controller 只轉發，service 維護預約人數 + 轉換）
...
public FarmTripOrderResponse bookSession(Integer sessionId, FarmTripOrderRequest request) {
    ...
    // attendance 是目前有效預約人數，預約成功後由後端累加
...
```
- **`attendance` 不由前端設定**：它代表目前有效預約人數，預約成功才由後端累加。
- **`@Transactional`**：確保建立訂單和更新 `attendance` 要嘛一起成功，要嘛一起失敗。

### 白話總結
controller 收 Request（先 `@Valid`）→ 丟給 service。service 確認場次存在且開放中 → 建立訂單 → 把該筆 `numPeople` 累加到場次的 `attendance` → 轉成 Response 回傳。
...
<a id="9"></a>
## 9. `POST /farmer/farm-trips` — 小農新增活動並送審 ⭐
...
前端 ──POST + JSON──▶ FarmerFarmTripController.createFarmTrip(farmTrip)
                     ...
                     DB: INSERT INTO farm_trip ...（status 一定是 PENDING）
                          ├─ 建立審核紀錄：status=PENDING、adminId=null
...
### 白話總結
新增活動時，後端把「狀態、評論統計」強制設成初始值（PENDING、0、0），再建立一筆 `adminId=null` 的待審核紀錄。前端只能決定標題、介紹這類欄位。

---

<a id="10"></a>
## 10. `PUT /farmer/farm-trips/{tripId}` — 小農修改活動並重新送審
...
前端 ──PUT + JSON──▶ FarmerFarmTripController.updateFarmTrip(tripId=5002, farmTrip)
                    ...
                    ServiceImpl.updateTrip()
                         ...
                         │     farmerId、評論統計 → 不動
                         ├─④ status 改回 PENDING 並儲存活動
                         └─⑤ 新增一筆 PENDING 審核紀錄（adminId=null）
...
public FarmTrip updateTrip(Integer tripId, FarmTrip trip) {
    ...
    if (db.getStatus() == FarmTripStatus.PENDING) {
        throw new IllegalStateException("活動已在待審核中");
    }
...
body: { "farmTripStart":"2026-09-01T09:00:00", "farmTripEnd":"..." }
```
...
前端 ──POST + JSON──▶ FarmerFarmTripController.createSession(tripId=5001, session)
                     ...
                     ServiceImpl.createSession()
                          ...
                          │ session.setAttendance(0)          ← 新場次尚無預約
...
public FarmTripSession createSession(Integer tripId, FarmTripSession session) {
    ...
    session.setAttendance(0);                    // 預約人數由後端維護
...
### 白話總結
新增場次，後端用網址的活動 id 綁定，預約人數歸零，沒給狀態就預設 ACTIVE，再存。
...
前端 ──PUT + JSON──▶ FarmerFarmTripController.updateSession(sessionId=6001, session)
                    ...
                    ServiceImpl.updateSession()
                         ...
                         │ ② 覆蓋時間4欄、sessionStatus（attendance 不讓前端修改）
...
前端 ──POST + JSON──▶ AdminFarmTripController.auditTrip(tripId=5001, auditRequest)
                     ...
                     ServiceImpl.auditTrip()  【@Transactional】
                          ...
                          ├─① 活動要存在：findById(5001)
                          │
                          ├─② 找最新一筆 status=PENDING 的審核紀錄
                          │
                          ├─③ 更新原本的待審紀錄：
                          │     audit.setAdminId / status / reason
                          │     audit.setUpdatedAt(現在)
                          │     auditRepository.save(audit)
                          │     → DB: UPDATE farm_trip_audits ...
                          │
                          └─④ 連動改活動狀態：
...
```
- **`@Transactional`**：更新審核紀錄、改活動狀態兩步綁一起，不會只成功一半。

### 白話總結
審核做兩件事：更新小農送審時建立的 `PENDING` 紀錄 + 依結果連動把活動改成 ACTIVE 或 REJECTED。若被拒絕，小農修改後再次送出會新增下一筆 `PENDING` 審核歷史。
